...
# Public `_search` names used by the current Materials Project summary API.  Keeping a
# closed allow-list is intentional: silently ignoring a misspelled scientific filter can
# change a screening result much more dangerously than returning an error.
...
_LIST_FILTERS = frozenset(
    {"formula", "chemsys", "elements", "exclude_elements", "material_ids", "has_props"}
)
_BOOLEAN_FILTERS = frozenset(
    {"is_stable", "theoretical", "is_metal", "is_gap_direct", "deprecated"}
)
_CONTROL_KEYS = frozenset({"max_records", "limit", "page_size", "fields"})

# Correct fields in the current MP SummaryDoc.  Elastic moduli are dictionaries whose
# `vrh` members are unpacked during normalization below; `bulk_modulus_vrh` and
# `shear_modulus_vrh` are not SummaryDoc fields.
_DEFAULT_FIELDS = (
...
# Summary property -> broad MP property-origin document.  These are task/source pointers,
# not claims about functional or calculation method.
...
class MaterialsProjectConnector(Phase11MaterialsProjectConnector):
    ...
    def fetch(self, **query: Any) -> FetchResult:
        ...
        while len(records) < max_records:
            ...
            if not first_meta:
                ...
                try:
                    total_doc = (
                        int(first_meta.get("total_doc"))
                        if first_meta.get("total_doc") is not None
                        else None
                    )
        ...
        version = (
            first_meta.get("db_version")
            or first_meta.get("database_version")
            or first_meta.get("data_version")
        )
        ...
        if not complete:
            notes.append(
                f"Result set was capped at {len(records)} of "
                f"{total_doc if total_doc is not None else 'unknown'} records."
            )
    ...
    def normalize(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        # Current SummaryDoc exposes elasticity as dictionaries, for example
        # {"voigt": ..., "reuss": ..., "vrh": ...}.  Convert only the explicitly
        # named VRH value into the Phase-11 normalized contract.
