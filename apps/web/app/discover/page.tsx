...
type Candidate={
  rank:number;material_id:string;display_name:string|null;chemical_formula:string|null;elements:string[];
  is_stable:boolean|null;is_theoretical:boolean|null;crystal_system:string|null;space_group_symbol:string|null;
  constraint_summary:{pass:number;fail:number;unknown:number;evidence_coverage:number};
  constraint_evaluations:Evaluation[];observations:Observation[];materials_project_origins:Array<Record<string,unknown>>;last_updated:string|null;
};
type DiscoveryResponse={
  preview_only:boolean;persisted:boolean;source:string;source_data_kind:string;warning:string;
  ranking_policy:{type:string;order:string[]};provider_version:string|null;records_screened:number;candidates_returned:number;candidates:Candidate[];
};
...
function fmt(value:number|null|undefined,unit:string){return typeof value==="number"?`${Number(value.toFixed(4))} ${unit}`:"—"}

export default function MaterialsDiscoveryPage(){
  const [form,setForm]=useState(initial);const [result,setResult]=useState<DiscoveryResponse|null>(null);const [loading,setLoading]=useState(false);const [error,setError]=useState("");
  const [projects,setProjects]=useState<ProjectSummary[]>([]);const [projectId,setProjectId]=useState("");const [adopting,setAdopting]=useState("");const [adopted,setAdopted]=useState<Record<string,AdoptResponse>>({});const [adoptError,setAdoptError]=useState("");
  ...
  async function discover(e:FormEvent){e.preventDefault();setLoading(true);setError("");setResult(null);setAdopted({});try{const max=Math.max(1,Math.min(100,form.limit));const response=await api<DiscoveryResponse>("/external-data/materials-project/discover",{method:"POST",body:JSON.stringify({constraints,formula:form.formula.trim()||null,chemsys:form.chemsys.trim()||null,elements:split(form.elements)||null,exclude_elements:split(form.excludeElements)||null,is_stable:form.stableOnly?true:null,theoretical:null,stable_preferred:true,max_candidates:max,search_pool:Math.max(max,Math.min(500,max*4))})});setResult(response)}catch(e){setError(e instanceof Error?e.message:"Materials Project discovery failed.")}finally{setLoading(false)}}
  ...
  return <div className="workspace-page">
    <div className="workspace-hero compact"><div><span className="workspace-kicker">Materials Project · evidence-aware discovery</span><h1>Find candidates, then deliberately admit them into a replacement mission.</h1><p>Search live computed records against explicit constraints. Screening remains read-only until you select a candidate and adopt it into a project.</p></div></div>
    <section className="command-panel" style={{marginBottom:20}}>
      <div className="panel-title-row"><div><span className="panel-kicker">Search definition</span><h2>Constraint-aware external screening</h2></div><span className="count-chip">{constraints.length} constraints</span></div>
      ...
      </div><div style={{display:"flex",gap:14,alignItems:"center",marginTop:18,flexWrap:"wrap"}}><label style={{display:"flex",gap:8,alignItems:"center"}}><input type="checkbox" checked={form.stableOnly} onChange={e=>setForm({...form,stableOnly:e.target.checked})}/><span>Stable materials only</span></label><button className="btn" disabled={loading||constraints.length===0}>{loading?"Searching…":"Discover candidates"}</button><button className="btn btn-secondary" type="button" onClick={()=>{setForm(initial);setResult(null);setError("")}}>Reset</button></div></form>
      <p className="muted" style={{marginTop:14}}>Materials Project values remain <strong>computed database evidence</strong>. They are not measurements, experimental validation, or acceptance decisions.</p>{error&&<div className="panel-empty"><strong>Discovery unavailable.</strong><p>{error}</p></div>}
    </section>

    {result&&<><section className="decision-outcome-kpis" style={{marginBottom:20}}><article className="neutral"><small>Screened</small><strong>{result.records_screened}</strong><span>External records</span></article><article className="advance"><small>Returned</small><strong>{result.candidates_returned}</strong><span>Ranked candidates</span></article><article className="hold"><small>Unresolved</small><strong>{result.candidates.filter(c=>c.constraint_summary.unknown>0).length}</strong><span>Need more evidence</span></article><article className="reject"><small>Failures</small><strong>{result.candidates.filter(c=>c.constraint_summary.fail>0).length}</strong><span>Constraint violations</span></article></section>
      <section className="command-panel" style={{marginBottom:20}}><div className="panel-title-row"><div><span className="panel-kicker">Candidate admission</span><h2>Choose the replacement mission</h2></div><span className="count-chip">explicit write step</span></div>{projects.length?<div style={{display:"flex",gap:12,alignItems:"end",flexWrap:"wrap"}}><label style={{minWidth:300}}><small>Replacement project</small><select value={projectId} onChange={e=>setProjectId(e.target.value)}>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>{projectId&&<Link className="btn btn-secondary" href={`/projects/${projectId}/replacement`}>Open mission</Link>}</div>:<p className="muted">Create a replacement project before adopting candidates. Discovery itself remains available.</p>}<p className="muted">Adoption re-fetches the selected MP record server-side, persists its snapshot/provenance/evidence chain, then adds it as a proposed candidate. Preview values are never trusted as write input.</p>{adoptError&&<div className="panel-empty"><strong>Adoption failed.</strong><p>{adoptError}</p></div>}</section>
      <section style={{display:"grid",gap:16}}>{result.candidates.map(c=>{const s=c.constraint_summary,a=adopted[c.material_id];return <article className="command-panel" key={c.material_id}><div className="panel-title-row"><div><span className="panel-kicker">#{c.rank} · {c.material_id}</span><h2>{c.display_name||c.chemical_formula||c.material_id}</h2><p className="muted">{c.chemical_formula||"formula unavailable"} · {c.is_stable===true?"stable":c.is_stable===false?"not source-stable":"stability unknown"} · {c.is_theoretical===true?"theoretical entry":c.is_theoretical===false?"non-theoretical entry":"theoretical flag unknown"}</p></div><span className={`count-chip ${s.fail?"reject":s.unknown?"hold":"advance"}`}>{s.fail?"SCREEN FAIL":s.unknown?"UNRESOLVED":"SCREEN PASS"}</span></div>
        <div style={{display:"flex",gap:18,flexWrap:"wrap",margin:"12px 0"}}><span><strong>{s.pass}</strong> PASS</span><span><strong>{s.fail}</strong> FAIL</span><span><strong>{s.unknown}</strong> UNKNOWN</span><span><strong>{pct(s.evidence_coverage)}</strong> coverage</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:14}}><div><small>Band gap</small><div>{fmt(observation(c,"band_gap"),"eV")}</div></div><div><small>Density</small><div>{fmt(observation(c,"density"),"g/cm³")}</div></div><div><small>Energy above hull</small><div>{fmt(observation(c,"energy_above_hull"),"eV/atom")}</div></div><div><small>Bulk modulus</small><div>{fmt(observation(c,"bulk_modulus"),"GPa")}</div></div><div><small>Shear modulus</small><div>{fmt(observation(c,"shear_modulus"),"GPa")}</div></div></div>
        <div className="requirement-mini-table"><div><span>Requirement</span><span>Screening result</span></div>{c.constraint_evaluations.map((r,i)=><div key={`${r.property}-${i}`}><span>{r.property}{r.minimum!==null?` ≥ ${r.minimum}`:""}{r.maximum!==null?` ≤ ${r.maximum}`:""}{r.value!==null?` · observed ${Number(r.value.toFixed(4))}`:" · no value"}</span><b className={`cell-state ${r.status}`}>{r.status.toUpperCase()}</b></div>)}</div>
