...
export default function ValidationPlanPage(){
  ...
  useEffect(()=>{
    if(!programId){setActions([]);setGaps(null);return;}
    ...
    ]).then(([a,g])=>{setActions(a);setGaps(g)}).catch(e=>setError(e instanceof Error?e.message:"Could not build validation plan.")).finally(()=>setLoading(false));
  },[programId]);
  ...
  return <div className="workspace-page">
    ...
    <section className="command-panel" style={{marginBottom:20}}>
      ...
      <p className="muted" style={{marginTop:12}}>Planning never starts a prediction, simulation, experiment, supplier check, or regulatory check. Execution remains an explicit action in the corresponding controlled workflow.</p>
    ...
    {loading?<div className="command-panel panel-empty">Building evidence-resolution plan…</div>:programId&&<>
      <section className="decision-outcome-kpis" style={{marginBottom:20}}>
        ...
        <article className="neutral"><small>Candidates</small><strong>{candidateCount}</strong><span>Represented in action queue</span></article>
      </section>
