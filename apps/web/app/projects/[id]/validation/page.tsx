...
export default function CandidateValidationPage({params}:{params:Promise<{id:string}>}){
  ...
  return <div className="grid">
    ...
    <div className="card card-pad"><div className="eyebrow">Decision context</div><div className="grid grid-2">
      <label className="label">Project candidate<select className="select" value={candidateId} onChange={e=>{setCandidateChoice(e.target.value);setSelectedSample("")}}>{candidates.data.items.map(c=><option key={c.id} value={c.id}>{c.display_name} · {c.candidate_kind}</option>)}</select></label>
      <label className="label">Material role<select className="select" value={roleId} onChange={e=>setRoleChoice(e.target.value)}>{roles.data?.map(r=><option key={r.id} value={r.id}>{r.application_name} · {r.component_name} · {r.display_name}</option>)}</select></label>
