...
export default function PredictionLabPage({params}:{params:Promise<{id:string}>}) {
  ...
  const runs=useQuery({queryKey:["prediction-runs",id],queryFn:()=>api<PredictionRun[]>(`/replacement-projects/${id}/prediction-runs`)});
  const [modelId,setModelId]=useState<string>(""); const selectedModelId=modelId||models.data?.find(m=>m.status==="approved")?.id||models.data?.[0]?.id||"";
  const versions=useQuery({queryKey:["prediction-model-versions",selectedModelId],queryFn:()=>api<PredictionModelVersion[]>(`/prediction-models/${selectedModelId}/versions`),enabled:!!selectedModelId});
  ...
  if(project.isLoading||models.isLoading||candidates.isLoading) return <div className="empty">Loading Prediction Lab…</div>;
  ...
  return <div className="grid">
    ...
    <div className="card card-pad"><div className="eyebrow">Model registry</div><h2>Choose an approved immutable model version</h2><div className="grid grid-2">
      ...
      <label>Version<select value={selectedVersion?.id??""} onChange={e=>{setVersionId(e.target.value);setPreview(null)}}>{versions.data?.map(v=><option key={v.id} value={v.id}>v{v.version} · {v.target_property_key} · {v.approved_at?"approved":"draft"}</option>)}</select></label>
      <label>Target temperature (°C)<input type="number" value={temperature} onChange={e=>{setTemperature(Number(e.target.value));setPreview(null)}}/></label>
      <div><span className="kicker">Feature schema</span><p>{selectedVersion?.feature_schema_version??"—"}</p><span className="kicker">Uncertainty method</span><p>{selectedVersion?.uncertainty_method??"—"}</p></div>
    </div>{selectedVersion&&<><DemoModelWarning text={String(selectedVersion.immutable_metadata?.demo_warning??"")}/><p className="muted">Artifact checksum <code>{selectedVersion.artifact_checksum}</code></p></>}</div>
    <div className="card card-pad"><div className="eyebrow">Prediction preview</div><h2>Assess applicability before inference</h2><p className="muted">{targets.length} bounded candidate target(s). Preview persists no PropertyPrediction records.</p><div style={{display:"flex",gap:8}}><button className="btn btn-secondary" disabled={!selectedVersion||!targets.length||previewRun.isPending} onClick={()=>previewRun.mutate()}>Preview only</button><button className="btn" disabled={!preview?.valid||!preview.expected_model_executions||execute.isPending} onClick={()=>execute.mutate()}>Execute prediction run</button></div>
