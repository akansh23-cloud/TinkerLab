...
def create_or_replace_identity(db: Session, *, material_id: str, payload: MaterialIdentityV13Create, review_required: bool = True) -> MaterialIdentityV13:
    ...
    identity_fp = engineering_identity_fingerprint(
    ...
    identity = db.query(MaterialIdentityV13).filter(MaterialIdentityV13.material_id == material_id).one_or_none()
    if identity is None:
...
def materialize_legacy_identity(db: Session, material: Material) -> MaterialIdentityV13:
    existing = db.query(MaterialIdentityV13).filter(MaterialIdentityV13.material_id == material.id).one_or_none()
...
def find_identity_conflicts(db: Session, material_ids: Iterable[str]) -> list[IdentityConflict]:
    ids = list(dict.fromkeys(material_ids))
    identities = db.query(MaterialIdentityV13).filter(MaterialIdentityV13.material_id.in_(ids)).all() if ids else []
    ...
    for comp_fp, group in by_composition.items():
        for index, left in enumerate(group):
            for right in group[index + 1:]:
                ...
                if differing:
                    conflicts.append(IdentityConflict(left.material_id, right.material_id, comp_fp, differing, IdentityConflictSeverity.BLOCKING))
    return conflicts
...
def assert_no_identity_conflation(db: Session, material_ids: Iterable[str]) -> None:
    ...
    if conflicts:
        summary = "; ".join(f"{item.left_material_id} vs {item.right_material_id}: {','.join(item.differing_dimensions)}" for item in conflicts)
...
def assert_directional_property_has_direction(identity: MaterialIdentityV13, *, property_key: str, direction: str | None) -> None:
    if property_direction_is_required(identity, property_key) and not direction:
        raise ValueError(f"property '{property_key}' is direction-sensitive for material {identity.material_id}; a scalar value without direction is not admissible evidence")
